...
- **Every function, class, and module gets a comment saying WHY it exists and what it is used for — never a step-by-step narration of the body.** See the repo's `CLAUDE.md`. The code already states its steps; a step-by-step comment is redundant when written and wrong after the first edit. Record instead what a reader cannot recover from the implementation: the reason it exists, the caller it serves, why it is built this way rather than the obvious alternative, which constants are load-bearing and what breaks if they change, and any distinction a reader would otherwise flatten (e.g. `None` meaning "not evaluable" versus `0.0` meaning "evaluated, no match"). Where this plan's reference code omits a docstring, add one — the omission is a gap in the plan, not a licence to skip it.
