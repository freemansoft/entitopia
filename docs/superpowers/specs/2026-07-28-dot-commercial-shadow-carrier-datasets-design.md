...
- **No natural single-column unique key exists in any of the three datasets** (no `id_field` candidate). This matches the `crashes` dataset's existing, already-accepted situation (documented as a known quirk in the inspections-per-unit plan: no `id_field` → Elasticsearch auto-generates a random `_id` on every populate, so re-running `index-populate` against the same day's index without deleting it first doubles the document count instead of overwriting). Note this is a genuine constraint of `phase_index_populate.py`, not a design oversight to fix here: `_id` is fixed from `record[id_field]` in the raw CSV row _before_ the ingest pipeline runs, so an ingest-time `fingerprint` processor **cannot** be used to synthesize an `_id` (`crashes-ingestion-setup` originally carried one for exactly this purpose; it was removed as dead weight once this was confirmed — see the "Validation findings" section below) — only a literal source-CSV column can. None of the three datasets has one. **Same operational consequence as `crashes`: re-running `index-populate` for these three without deleting the current day's index first will double-count.** Accepted, consistent with existing precedent, not fixed here.
  - **Superseded 2026-07-29 — see "`id_field` fix" section below.** Re-inspecting the raw CSVs directly (not just the Socrata column list) found that `boc3-agents.docket_number` actually _is_ a natural single-column unique key — the claim above was based on column names alone, not measured cardinality. It was wired up, along with a new composite-key mechanism for the two datasets that genuinely have no single-column key.
...
Full end-to-end validation was run: all three indexes populated at full production scale — `auth-history` (4,941,925 docs; the ~65-70 minute estimate from a slow-network measurement dropped to ~1 minute once network conditions improved, so it was pulled in full rather than left at validation scale), `out-of-service-orders` (394,963 docs), and `boc3-agents` (1,860,604 docs) — enrichment policies/pipeline rebuilt against all three, and spot-checked against real carrier documents. Findings worth carrying forward:

- **The "no `id_field`" limitation is real, not just theoretical.** Populating `out-of-service-orders`/`boc3-agents` a second time (validation sample, then full scale) against the same day's index without deleting it first produced exactly the predicted accumulation: `394,963 + 5,000 = 399,963` and `1,860,604 + 5,000 = 1,865,604` documents. The fix used was simply deleting the day's index before a same-day rerun (matching the `crashes` precedent) — done here before finalizing the full-scale counts above.
  - **Three candidate fixes were considered for the underlying limitation, and none were built:**
    1. _Give `dot_number` a different type on fetch_ — doesn't apply here. That's the unrelated `crashes.dot_number` float-suffix bug (caused by nulls forcing `float64` inference), and none of these three datasets has that problem (see above — zero nulls).
    2. _Reuse `crashes`' existing fix as-is_ — doesn't work. That fix bundles two things: the float-cast (not needed here) and a `fingerprint` ingest processor intended to produce a composite ID. The fingerprint approach doesn't actually work as `_id` for `crashes` either — it's a pre-existing, already-documented bug, since `_id` is set from `record[id_field]` on the raw CSV row **before** any ingest pipeline runs (`phase_index_populate.py`), so no ingest-time processor can ever populate it. Copying this over would just copy a broken mechanism.
    3. _Compute a genuine composite key_ (e.g. `dot_number`+`oos_date` for `out-of-service-orders`) _as a real `id_field`_ — the only approach that would actually work, but requires a Python-level code change (compute the composite column in the dataframe before the bulk-index call, in `phase_index_populate.py` or a preprocessing step) since `id_field` can only reference a literal source-CSV column, not an ingest-pipeline-derived one. Not built.
  - **Conclusion at the time: no fix needed given how this project actually runs these datasets** — deleting the day's index before a same-day rerun was judged a sufficient workaround. **Superseded 2026-07-29 — see "`id_field` fix" section below.** Once asked to extend this same uniqueness analysis to all 7 datasets, `boc3-agents.docket_number` turned out to be a natural single-column key after all (the "no natural key" claim above was based on the column list, not on actually measuring cardinality against the real CSV), and a real composite-key mechanism was built and wired up for `auth-history` and `out-of-service-orders`, which genuinely have no single-column key.
...
- ~~No `id_field` on any of the three new datasets~~ — **fixed 2026-07-29**, see "`id_field` fix" section below. All three now have a real `id_field` (natural for `boc3-agents`, composite for `auth-history`/`out-of-service-orders`), so a same-day rerun overwrites instead of duplicating.
...
(The "no `id_field` → duplicate on same-day rerun" limitation was originally listed as out of scope here; it was fixed 2026-07-29 — see "`id_field` fix" below.)

## `id_field` fix (2026-07-29)

The "no fix needed" conclusion above held only as long as manual index deletion before every same-day rerun was an acceptable workaround. Asked to extend the composite-key idea to all 7 datasets (not just the two originally flagged), a direct uniqueness check against the real CSVs — not just the column list — found the "no natural key" claim above was wrong for one dataset, and gave exact figures for the rest:

| Dataset                 | Existing `id_field`? | Measured uniqueness                                                                                                                                                      |
| ----------------------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `carriers`              | `dot_number`         | 100% unique (2,085,534/2,085,534) — already correct                                                                                                                      |
| `inspections`           | `inspection_id`      | 100% unique (5,647,567/5,647,567) — already correct                                                                                                                      |
| `inspections-per-unit`  | `insp_unit_id`       | 100% unique (9,620,293/9,620,293) — already correct                                                                                                                      |
| `crashes`               | _(none)_             | `crash_id` alone is 100% unique (333,186/333,186) — a natural key that was simply never wired up                                                                         |
| `boc3-agents`           | _(none)_             | `docket_number` alone is 100% unique (1,860,604/1,860,604) — same, natural key never wired up                                                                            |
| `out-of-service-orders` | _(none)_             | No single field unique; `dot_number`+`oos_date`+`oos_reason`+`status`+`rescind_date` together are 100% unique (394,963/394,963)                                          |
| `auth-history`          | _(none)_             | Even all 9 columns combined leave 10,510 exact full-row duplicates baked into the source itself — no key can separate true duplicates from each other, and none needs to |

**Implementation.** `phase_index_populate.py`'s `record_action()` previously did a single-key lookup, `record[id_field]`. Added a `compute_id(record, id_field)` method: if `id_field` is a JSON list, joins the named fields' values with `|` to build the `_id`; if it's a string, behaves exactly as before (verified against `carriers` to confirm zero behavior change for existing string-`id_field` datasets). `id_field` unset still falls through to `KeyError` → ES auto-generated `_id`, unchanged.

**Config changes** — no other code touched:

- `crashes/index-config.json`: `"id_field": "crash_id"`
- `boc3-agents/index-config.json`: `"id_field": "docket_number"`
- `out-of-service-orders/index-config.json`: `"id_field": ["dot_number", "oos_date", "oos_reason", "status", "rescind_date"]`
- `auth-history/index-config.json`: `"id_field"` set to a list of all 9 columns

**Applied and verified against the live cluster**: each day's index was deleted and repopulated. Results matched the uniqueness table exactly — `crashes`, `boc3-agents`, and `out-of-service-orders` came back at identical counts (their keys were already fully unique, so nothing collapses), and `auth-history` came back at 4,931,415 (down from 4,941,925 — exactly the 10,510 true duplicates collapsing into single documents, which is correct since they're byte-identical rows).

**Not done**: the `carriers`-side enrichment policies/pipeline were not rebuilt as part of this fix (that's the separate, still-pending full `carriers` repopulate noted elsewhere) — this fix only concerns the 4 raw datasets' own `_id` behavior on (re)populate.
