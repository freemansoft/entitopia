...
- **The "no `id_field`" limitation is real, not just theoretical.** Populating `out-of-service-orders`/`boc3-agents` a second time (validation sample, then full scale) against the same day's index without deleting it first produced exactly the predicted accumulation: `394,963 + 5,000 = 399,963` and `1,860,604 + 5,000 = 1,865,604` documents. The fix used was simply deleting the day's index before a same-day rerun (matching the `crashes` precedent) — done here before finalizing the full-scale counts above.
  ...
  - **Conclusion: no fix is needed given how this project actually runs these datasets.** Deleting the day's index before a same-day rerun is a complete, sufficient workaround for the specific failure mode (double-population within one calendar day) — it's the same operating pattern `crashes` already lives with, and it's what was done here to get the clean final counts above. The one gap it doesn't close is retry-safety after a run that fails partway through (no way to safely "resume" — you'd still delete and restart), but since all three datasets are unwindowed full pulls by design, "delete and reload everything" is already the normal recovery path regardless, so that gap costs nothing extra in practice. A composite-key fix would only start to matter if incremental/partial loading were ever added — not planned here.
