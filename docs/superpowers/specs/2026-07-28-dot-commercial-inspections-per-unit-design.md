...
- `DOT-Commercial/configuration/crashes/index-mappings.json` (**new file** — crashes currently has none): `{"properties": {"dot_number": {"type": "long"}}}`. **Correction (discovered during implementation, 2026-07-28):** this mapping alone does NOT fix the bug. Elasticsearch `match`-type enrich policies build their internal index by copying each source document's raw `_source` verbatim and always index the match field as `keyword` — mapping coercion on the live index changes how the field is searched there, but never rewrites `_source`, which is what the enrich index is built from. Since pandas writes `dot_number` as a float (`"4023446.0"`) for any column containing nulls, that literal string — not `"4023446"` — is what lands in `_source` and therefore in the enrich index, and it never term-matches carriers' clean integer `dot_number`. The actual fix additionally requires a `convert` ingest processor (added to the existing `crashes-pipeline-000001`) that rewrites `dot_number` to a real JSON integer before `_source` is stored — a pipeline-config change, not a Python code change, but not "mapping alone" either. See `docs/superpowers/plans/2026-07-28-dot-commercial-inspections-per-unit.md` Task 5 for the corrected approach.
