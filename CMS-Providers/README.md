...
1. No `entity-match` step. The phonetic and cleaning analyzers are configured on names and addresses here but nothing queries them — this project loads and analyzes, it does not yet match. A `hospital-duplicates` step is designed in [the analysis portability spec](../docs/superpowers/specs/2026-08-16-config-driven-analysis-portability-design.md), which needs the framework's `all-entities` population mode to exist first.
