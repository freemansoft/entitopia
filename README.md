...
1. **A scored pair does not record which tokenization produced it, so it stops being interpretable once its source index is gone.** Indexes are stamped at creation (`phase_index_creation` writes `_meta.analysis_fingerprint`), and the sweep checks the _source_ index's stamp against the analyzers on disk — deliberately advisory, since sweeping an older index to reproduce a past result is legitimate. But that check is a **log line at run time**, and the pairs are the durable artifact: each one carries `run_id` and `generated_at` and no fingerprint, and the candidates index gets no `_meta` of its own (the stamp is computed from an index's own analyzers, and the pairs index has none). So a pair on disk cannot answer "which analyzers scored this?" — the answer lives only in whatever log that run wrote, and in the source index continuing to exist.

   That matters because the two are on a collision course with the retention item below: every figure in `DOT-Commercial/README.md` defends itself by quoting fingerprint `0595ca890d9ec6fb`, and that defense holds only while the carriers index that carries the stamp is still on the cluster. Prune it and the pairs it produced become unverifiable, silently — the scores still look fine.

   The pattern already exists elsewhere: `scripts/measure_crash_lift.py` writes the source index's `analysis_fingerprint` onto its summary row. (The live `chameleon-validation` index shows none because it was created three and a half hours before that code landed, not because the code omits it.) Fix by having `phase_entity_match` stamp the **source** index's fingerprint onto the candidates index's `_meta` at creation, and consider carrying it per pair as well, since a pair is routinely read on its own rather than through its index.

1. **A row whose `id_field` value is blank gets an auto-generated `_id`, so it duplicates on every reload into an existing index.** `id_utils.compute_id` returns the field's _value_, and `phase_index_populate` has already replaced `NaN` with `None` by then, so `_id` is `None` and Elasticsearch generates a fresh random one each run. The `KeyError` fallback below it covers an _absent column_, not a present-but-blank value, which is why nothing reports this. **Measured 2026-08-13**: `boc3_agents.csv` carries exactly one blank `docket_number` among 1,860,604 rows, and re-populating the existing index left it holding 1,860,605 documents — the live index still does. Harmless at one row, silently cumulative at more, and it defeats the "re-running an ingest overwrites rather than duplicates" guarantee `compute_id`'s own docstring makes. Fixing it needs a decision rather than a patch: either fail the load, since a row with no key cannot be addressed or corrected later, or fall back to a deterministic hash of the whole row so the value is at least stable across runs. Note that a full rebuild into a _fresh_ index does not accumulate — only re-populating an existing one does.
