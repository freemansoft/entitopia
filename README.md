...
1. **A composite `_id` renders a missing component as the literal string `"None"`.** Surfaced 2026-08-15 while closing the blank-key item below, and left alone deliberately rather than overlooked. `compute_id` joins `str(value)` per component, so a row with an absent keyed column keys as `1498477|2007-09-10|...|INACTIVE|None` — **221,812 of 395,269 documents (56.1%) in the live `out-of-service-orders` index**, since `rescind_date` is usually absent. Two consequences: a Python repr leaks into the key space, and a row whose column genuinely holds the string `"None"` gets the same `_id` as one where it is missing. Rendering it as `""` instead is a two-character change; what makes this an item rather than a fix is that it re-keys all 221,812 documents, so the next `index-populate` into an _existing_ index writes them a second time under their new ids. Fixing it means pairing the change with a rebuild into a fresh index, which is why it is not folded into the closed item.
...
1. **A scored pair now records which index, analyzed which way, produced it.** The check was already there and was only a log line; the pairs are what survives the run. `phase_entity_match` reads the source index's stamp once in `_preflight` — the same read the staleness check uses, so the value reported and the value stored cannot drift — and carries it two ways: `_meta.source_index` / `_meta.source_analysis_fingerprint` on the candidates index, and `source_index` / `analysis_fingerprint` on every pair document. Both, not either: a pair is routinely read on its own — pulled by `_id`, exported into a review sample, quoted in a README — and at that point the index's `_meta` is not in the reader's hands.

   Two shapes worth keeping. The index-level keys are `source_*` because **the candidates index has no analyzers of its own**; a bare `analysis_fingerprint` there would be read as this index's own stamp, compared against the wrong config, and report a mismatch on every run. And a source index that predates the stamp leaves the field **absent rather than null**, so nobody can later quote a fingerprint the pair never had — the same distinction `_check_analysis_fingerprint` already draws between "unknown" and "wrong".

   Verified 2026-08-15 against the live cluster: `put_mapping(index=..., meta=...)` round-trips on the pinned 9.4.1 server, and `carriers-2026.08.13-000001` carries `0595ca890d9ec6fb` — the value every figure in `DOT-Commercial/README.md` cites — so pairs from the next sweep defend themselves whether or not the carriers index survives. Existing candidates indexes are **not** retro-stamped; they predate this and stay dependent on their source index.
