...
```mermaid
flowchart TB
    Sel[PopulationSelector<br/>the swept population] --> Seed
...
**`matching/`** — sweep mechanics: population selection, candidate retrieval, signal evaluation, scoring. Carries no dataset vocabulary. Everything a project knows about itself reaches it through `entity-match.json`: the entity key and the fields a pair summary shows (`entity`), the dates a gap is measured between (`lifecycle`), which records are swept and how a selector is composed (`population`), and which evidence is scored at what weight (`signals`).

This section used to carry a "known exception" recording that `matching/` was generic in structure but not in vocabulary — `CarrierDoc.dot_number`, and `matching/predecessors.py` hardcoding FMCSA field paths, the literal `"REVOKED"` and a `dot_number` sort. Both are gone; see the closed work items. What has **not** been demonstrated is the claim the change was made for: no project other than DOT-Commercial has yet configured a sweep, so "a new dataset is onboarded by writing configuration" is a property of the code that nothing has exercised. Treat it as untested until a second project does.
...
1. **`matching/` is dataset-agnostic, and DOT-Commercial's sweep is unchanged by the change.** `CarrierDoc.dot_number` became `EntityDoc.entity_key` read from `entity.key`; `matching/predecessors.py` became `matching/population.py`, with the four FMCSA selectors moved into project configuration and assembled from a closed four-clause menu — `nested-exists`, `term`, `all`, `any` — that stays in code. `agent` rarity generalized to a per-field rarity table, and the `vin-overlap` signal type was deleted in favour of `shared-token`, which the class was already registered under.
   ...
   **`nested-exists` is one clause kind rather than three composable ones on purpose.** Flattening is a known defect here, not a style preference: under an object mapping an ACTIVE 2015 order and an INACTIVE 2022 order jointly satisfied filters no single order met, and the temporal signal then reported a date from an element the selector never intended to match. If `exists`, `terms` and `range` could be combined freely, a project could rebuild that shape out of config that looks entirely reasonable.

   Two changes were made for correctness rather than portability and are worth knowing about independently. The dates a pair's gap spans lived in **two** places — on the `temporal` signal and again as literals where the phase computed the emitted `gap_days` — with nothing checking they agreed, so a pair could be scored on one pair of dates and reported with a gap measured between another; both now read one `lifecycle` block. And the emitted pair summary was a fixed five-field FMCSA list, so any other project's pairs would have carried five keys holding nulls.

   **One deliberate behavior change:** the gap window used to switch off when no `temporal` signal was configured, because the gate borrowed that signal's field paths; it now switches off when no `lifecycle` block is present. A config with the window set, a lifecycle block and no temporal signal therefore gates where it previously did not. That is the better reading — the window says which pairs are coherent enough to emit, not which signals happen to be configured — and it cannot affect DOT-Commercial, which configures both.

   **Verified 2026-08-17 against the live cluster**, sweeping `carriers-2026.08.13-000001` (stamped `0595ca890d9ec6fb`) with no reload in between, since reloading first would have conflated this work with the `dot_number` and composite-`_id` re-keying still queued above. All eleven metrics in `baseline-post-reload.json` were **exactly equal**, and the pair `_id` sets **identical at 75,537 each** — a separate check, because eleven aggregate counts agree just as readily when a pair is lost and another gained inside the same score band. The sweep's own figures corroborate from a third direction: 48,540 predecessors and 43,111 truncated candidate sets, both matching what `DOT-Commercial/README.md` records for the baseline run, so the config-defined `out-of-service` selector picks the same population the hardcoded one did. Full record in [the runbook](docs/superpowers/plans/2026-08-16-compatibility-gate-runbook.md).

   Metric equality was checked by direct comparison, **not** through `utils.sweep_compare.compare()`: that engine judges whether an intentional change moved the right metrics in the right direction, and its vocabulary has no way to say "must not change at all", so a real regression could have cleared it wearing `informational`.
