...
1. **`entity-match` thresholds are still uncalibrated, though it has now run against production data.** A full sweep over the July 2026 extract (2,085,534 carriers, 48,540 predecessors, 500 candidates each) emitted **421,613 pairs** from 22,316,105 candidates examined, of which 80.1% score below 0.50 — that band is noise. Applying the triage filter — re-registered within a year of shutdown, scoring ≥ 0.70, and sharing a VIN or phone/email — leaves **906 pairs**, of which 436 reuse the _byte-identical_ legal name.

   An earlier revision of this item cited "roughly 195 pairs" for that set. That figure could not be reproduced: the run behind it is no longer on the cluster, and its prose does not record the filter that produced it. The 906 above is stated with its filter so the next reader can re-derive it. A pre-change run of the same filter yielded **957**, but the two numbers are observations from two different runs, not a controlled before/after comparison, and this does not attribute the gap between them to the address normalization work. Of the 57 pairs present in one run and not the other, 38 lost their `vin-overlap`/`exact-identifier` clause entirely — a change no street analyzer can cause — which is itself evidence the runs differ on more than street matching. A controlled comparison would need two sweeps over the same extract that differ only in the street analyzers.
   ...
   **These figures predate a later correction to `unit_designator_stop` and are not exactly reproducible from the shipped config.** The sweep ran before seven spelled-out words (`space`, `office`, `pier`, `trailer`, `hangar`, `building`, `slip`) were removed from the stop list and `spc` was added. Those words occur on roughly 2,800 `phy_street` values out of 2,085,534, so the expected effect on a 421,613-pair sweep is in the hundreds — the magnitude and the sanity anchor above hold, the exact counts will not. Re-run the sweep before treating any figure here as a baseline for calibration.
...
1. **An index alias is never detached from the index it previously pointed at, so every reload doubles the data behind it.** `phase_index_creation` calls `put_alias`, which is purely additive; no `delete_alias` or `update_aliases` remove action exists anywhere in the codebase. Measured after a second `carriers` reload: `carriers-000001` resolved to two indexes and `_count` through it returned **4,171,070** against 2,085,534 real records. The results alias had accumulated three. This contradicts the top-level README's claim that a reload "can build a new index and swap without downtime", and the one-alias-one-index depiction in the diagrams below. Three consequences compound: a sweep reads a mixture of differently-analyzed tokens; `_preflight` iterates `mapping.body.values()` and breaks on the first entry, so its fingerprint and subfield checks inspect an arbitrarily chosen index; and because `CandidateFinder` excludes the predecessor by `_id`, the same carrier in the second index becomes its own perfect-scoring successor. Fix designed in [the atomic alias swap spec](../docs/superpowers/specs/2026-08-05-atomic-alias-swap-design.md) — one `update_aliases` call carrying paired remove/add actions. Affects all eight aliased steps here and all three in CMS-Providers.
